...
async def handler(event):
    ...
    if subscribed:
        ...
        if user_id in restricted_users:
            await unrestrict_member(chat.id, user_id)
...
async def monitor_restrictions():
    ...
    while True:
        ...
        to_remove = []
        for user_id, chat_id in [(uid, chat_id) for uid in restricted_users for chat_id in client.iter_dialogs() if chat_id.is_group]:
            try:
                if await is_subscribed(user_id):
                    await unrestrict_member(chat_id.id, user_id)
                    to_remove.append(user_id)
            except:
                continue
        for uid in to_remove:
            restricted_users.discard(uid)

async def main():
    await client.start()
    # بدء مهمة التحقق الدوري
